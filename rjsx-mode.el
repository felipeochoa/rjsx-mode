...
(define-derived-mode rjsx-mode js2-jsx-mode "RJSX"
  ...
  ;; \n is included to get around `comment-normalize-vars' and `comment-only-p'
...
(defun rjsx-ancestor (node predicate)
  "Return closest ancestor of NODE where PREDICATE is true.
Returns nil if no ancestor satisfies PREDICATE."
...
(defun rjsx-comment-region-function (beg end &optional arg)
  "JSX-aware commenting between BEG and END.
ARG is passed to `comment-region-default' when no special JSX
handling is needed; otherwise it is ignored."
...
(defun rjsx-uncomment-region-function (beg end &optional _)
  "JSX-aware uncommenting between BEG and END."
  (js2-mode-wait-for-parse
   (lambda ()
     (goto-char beg)
...
(defun rjsx-comment-dwim (arg)
  "RJSX implementation of `comment-dwim'.
If called on a region, this function delegates to
`comment-or-uncomment-region'.  If the point is not in a JSX
context, it delegates to the `comment-dwim', otherwise it will
comment the JSX AST node at point using the apppriate comment
delimiters.

ARG is as in `comment-dwim'.
